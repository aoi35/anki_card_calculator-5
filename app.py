...
# ===== 入力フォーム =====
input_time = st.text_input("昨日Inputに使った時間 / Yesterday's Input time (例: 1:00)", st.session_state['input_time'])
anki_time = st.text_input("昨日暗記カードにかかった時間 / Yesterday's Anki time (例: 0:15)", st.session_state['anki_time'])
...
factor = st.number_input("新規カード労力係数 / Effort Multiplier for New Cards ", min_value=1, value=st.session_state['factor'])
